...
async def check_lab_session_status(page, step="LAB CHECK"):
    """
    Check whether the Skills Boost lab session is clearly expired/ended.

    This intentionally uses only strong expiration signals. Generic text such as
    "Start Lab" is not treated as expiration because it can appear on normal
    lab pages before a session starts.
    """
    try:
    ...
    # Do not inspect Google authentication or Cloud Console pages as if they
    # were the Skills Boost lab page.
    skills_page = (
...
async def monitor_lab_session(monitor_page, stop_event, expired_event, error_holder):
    """
    Monitor the Skills Boost lab session independently while Cloud Console
    ...
    while not stop_event.is_set():
        try:
            ...
            if "Lab session is no longer active or has expired" in str(ex):
                error_holder.append(str(ex))
                expired_event.set()
                debug_log(step, "Lab session expired.")
                return
            error_holder.append(str(ex))
            expired_event.set()
            debug_log(step, f"Lab monitor error: {type(ex).__name__}: {ex}")
            return
        except Exception as ex:
            # A temporary monitor read/navigation problem should not stop the
            # main automation. Retry on the next cycle.
            debug_log(step, f"Temporary monitor error: {type(ex).__name__}: {ex}")

        try:
            await asyncio.wait_for(stop_event.wait(), timeout=5000)
...
async def ensure_lab_active(expired_event, error_holder):
    if expired_event.is_set():
        message = error_holder[0] if error_holder else "Lab session is no longer active or has expired."
...
async def wait_for_lab_dashboard(page, timeout_ms=600000):
    ...
    while asyncio.get_running_loop().time() < deadline:
        ...
        # Check the Skills Boost page for a clearly expired/ended lab.
        # Google authentication pages are intentionally excluded by the helper.
        ...
        google_page = (
            ...
            or "verifieren dat jij het bent" in body_lower
            or "verifiëren dat jij het bent" in body_lower
            or "doorgaan" in body_lower
        )
...
async def step3_enable_api(page, project_id, authuser):
    ...
    # Do not check Enable/Manage immediately after navigation.
    # Google may redirect this tab to accounts.google.com and wait for the
    # user to finish sign-in before returning to Cloud Console.
    try:
        await page.goto(api_url, wait_until="domcontentloaded", timeout=60000)
    except PlaywrightTimeoutError:
        # A navigation timeout can happen while Google is redirecting between
        # authentication and Cloud Console. Continue and inspect the real page.
        debug_log(step, "Initial navigation timed out; inspecting the current page.")

    auth_timeout_ms = 900000  # 15 minutes for manual Google authentication
    ...
    while asyncio.get_running_loop().time() < auth_deadline:
        ...
        if on_google_auth:
            ...
            if state != last_state or current_url != last_url:
                debug_log(
                    step,
                    "Google sign-in/verification page detected. "
            ...
            # Give the user/browser time to complete the interactive login.
            await page.wait_for_timeout(1500)
            continue
        ...
        if state != last_state or current_url != last_url:
            debug_log(
                step,
                f"Waiting for Google -> Cloud Console redirect. "
    ...
    # Cloud Console can still need several seconds to render the API page
    # after authentication. Wait for the actual API controls instead of using
    # a fixed short sleep.
    ...
    controls_deadline = asyncio.get_running_loop().time() + (controls_timeout_ms / 1000)
    ...
    while asyncio.get_running_loop().time() < controls_deadline:
        ...
        debug_log(step, f"Enable visible: {ev} | Manage visible: {mv} | Disable visible: {dv}")
...
async def step5_get_deployed_url(page, expired_event=None, error_holder=None):
    ...
    try:
        while asyncio.get_running_loop().time() < deadline:
            ...
            try:
                if await link_locator.is_visible(timeout=1500):
                    ...
                    if not final_url:
                        raise RuntimeError("run.app link is visible but its href is empty.")
                    return final_url
            except PlaywrightTimeoutError:
                pass
        ...
        raise PlaywrightTimeoutError("run.app URL was not detected within 120 seconds.")
    except Exception as ex:
    ...
    async with BROWSER_LOCK:
        async with async_playwright() as p:
            ...
            try:
                ...
                # Use a separate page in the same persistent context so the
                # Skills Boost session can be checked while the main page is
                # working inside Cloud Console.
                ...
                try:
                    ...
                    debug_log("LAB MONITOR", "Initial monitor navigation timed out; retrying on the next cycle.")
                ...
                if lab_monitor_task is not None:
                    lab_monitor_stop.set()
                    try:
                        await asyncio.wait_for(lab_monitor_task, timeout=3)
                    except (asyncio.TimeoutError, asyncio.CancelledError):
                        lab_monitor_task.cancel()
                        try:
                            await lab_monitor_task
                        except asyncio.CancelledError:
                            pass
                    except Exception as monitor_close_error:
                        debug_log("LAB MONITOR", f"Monitor shutdown error: {type(monitor_close_error).__name__}: {monitor_close_error}")

                if lab_monitor_page is not None:
                    try:
                        await lab_monitor_page.close()
                    except Exception as monitor_page_error:
                        debug_log("LAB MONITOR", f"Monitor page close error: {type(monitor_page_error).__name__}: {monitor_page_error}")
...
async def start(event):
    welcome_msg = (
        "Welcome to the Cloud Run creation bot.\\n\\n"
        "Lab 1 duration: 4 hours 30 minutes\\n"
        "Lab 1 link:\\n"
        "https://www.cloudskillsboost.google/focuses/20774?parent=catalog\\n\\n"
        "Lab 2 duration: 3 hours\\n"
        "Lab 2 link:\\n"
