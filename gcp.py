...
async def step1_welcome_screen(page):
    """Handle the optional Lab welcome screen without blocking on networkidle."""
    await page.wait_for_load_state("domcontentloaded", timeout=30000)
    await page.wait_for_timeout(1500)

    # Do not press Enter blindly: if Google shows a verification/login page,
    # Enter can interact with the wrong form. Only click the Lab welcome button
    ...
    try:
        ...
        print(f"Welcome screen was not clickable: {e}", flush=True)
...
async def process_sso_link(chat_id, sso_url):
    ...
    locale='en-US'
                ...
                await handle_error(
                    page, chat_id, "انتهى وقت الانتظار (Timeout)", str(e)
                )
            except Exception as e:
